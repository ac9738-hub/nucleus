...
test('normalizeBrowserUrl allows regular browser and in-app urls', () => {
  ...
  assert.equal(normalizeBrowserUrl('nucleus://engine'), 'nucleus://engine')
  assert.equal(normalizeBrowserUrl('about:blank'), 'about:blank')
})

test('normalizeBrowserUrl converts unsafe schemes into searches', () => {
  assert.equal(
