...
(function initArtifactActions(global) {
  ...
  function findCard(container, artifactId) {
    ...
    return Array.from(container.querySelectorAll(".artifact-card"))
      .find(card => card.dataset.artifactId === artifactId) || null;
  }
  ...
  function buildCard(artifact, handlers) {
    ...
    frame.title = `${artifact.title || "Artifact"} preview`;
    frame.dataset.artifactPreview = artifact.id || "";
