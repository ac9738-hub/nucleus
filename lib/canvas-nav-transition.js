...
function createCanvasNavTransition(deps) {
  ...
  function revealView(view, tab, window, reason) {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7283/ingest/c1155abf-8302-4940-9722-19bb0cae0569',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'5b3c30'},body:JSON.stringify({sessionId:'5b3c30',location:'canvas-nav-transition.js:revealView',message:'revealed',data:{reason,tabId:tab&&tab.id?tab.id:'',url:viewUrl(view),concealFlag:tab?Boolean(tab._nucleusConcealWebSource):false,runId:'post-fix'},timestamp:Date.now(),hypothesisId:'H4'})}).catch(()=>{});
    // #endregion
    log('ensure_revealed', {
      reason,
  ...
  function armPaintWait(t) {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7283/ingest/c1155abf-8302-4940-9722-19bb0cae0569',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'5b3c30'},body:JSON.stringify({sessionId:'5b3c30',location:'canvas-nav-transition.js:armPaintWait',message:'paint_wait_armed',data:{destUrl:t.destUrl||'',liveUrl:viewUrl(view),urlMatch:t.destUrl?urlsMatch(viewUrl(view),t.destUrl):null,reason:t.reason||'',tabId:t.tab&&t.tab.id?t.tab.id:''},timestamp:Date.now(),hypothesisId:'H9'})}).catch(()=>{});
    // #endregion
  }
